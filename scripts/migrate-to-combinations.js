 ...
 */
function generateCombinations(paramValues) {
 ...
 */
function findMainParameter(rows) {
 ...
 */
function getTiersForParams(rows, paramValues) {
    for (const row of rows) {
        const attrs = typeof row.attrs === 'string' ? JSON.parse(row.attrs) : row.attrs;
...
async function migrateService(serviceSlug) {
    ...
    for (const row of service.rows) {
        ...
        for (const [key, value] of Object.entries(attrs)) {
            // Skip system fields
            if (key === '_isMain' || !value)
                continue;
            if (!paramValues[key]) {
                paramValues[key] = new Set();
            }
            paramValues[key].add(value);
        }
    }
    console.log(`📋 Found parameters:`, Object.keys(paramValues));
    console.log(`📋 Parameter values:`, Object.fromEntries(Object.entries(paramValues).map(([k, v]) => [k, Array.from(v)])));
...
module.exports = { migrateService, main };
