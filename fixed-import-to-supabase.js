...
async function fixedImportData() {
  try {
    console.log('🚀 Начинаем исправленный импорт данных в Supabase...');
    
    // Проверяем, что база данных пустая
    const existingServices = await prisma.service.count();
    if (existingServices > 0) {
      console.log('⚠️ В базе уже есть данные! Очищаем...');
      ...
      try {
        await prisma.service.create({
          data: {
            ...
            isActive: Boolean(service.isActive), // Конвертируем в boolean
      ...
      try {
        await prisma.priceRow.create({
          data: {
            ...
            isActive: Boolean(priceRow.isActive), // Конвертируем в boolean
    ...
    for (const tier of tiers) {
      try {
        ...
        if (!existingPriceRowIdSet.has(tier.rowId)) {
          console.log(`⚠️ Пропускаем tier ${tier.id} - price row ${tier.rowId} не существует`);
          continue;
        }
      ...
      try {
        await prisma.menuTile.create({
          data: {
            ...
            isActive: Boolean(menuTile.isActive), // Конвертируем в boolean
      ...
      try {
        await prisma.whyArticle.create({
          data: {
            ...
            isActive: Boolean(whyArticle.isActive), // Конвертируем в boolean
